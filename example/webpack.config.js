...
module.exports = {
    ...
    entry: {
        index: ['./src/app.jsx'],
        lz: ['./src/page/lz.js'],
        encrypt: ['./src/page/encrypt.js'],
        mutation: ['./src/page/mutation.js'],
        performance: ['./src/page/performance.js'],
        'isomorphism-text': ['./src/page/isomorphism-text.js'],
        'view-onscroll': ['./src/page/view-onscroll.js']
    },
    output: {
    ...
    plugins: [
        ...
        new HtmlWebpackPlugin({
            template: './src/html/performance.html'
            , filename: 'performance.html'//可以使用hash命名
            , title: 'performance 性能 测试'
            , inject: 'body'//脚本包含到body 也可以写到head里面
            , chunks: ['manifest', 'commons', 'performance']//指定当前模板需要打入哪些js模块
            , minify: {//启用代码代码压缩
                removeComments: false,//移除注释
                collapseWhitespace: false//移除空格
            }
        ...
        new HtmlWebpackPlugin({
            template: './src/html/lz.html'
            , filename: 'lz.html'//可以使用hash命名
            , title: '数据压缩测试'
            , inject: 'body'//脚本包含到body 也可以写到head里面
            , chunks: ['manifest', 'commons', 'lz']//指定当前模板需要打入哪些js模块
            , minify: {//启用代码代码压缩
                removeComments: false,//移除注释
                collapseWhitespace: false//移除空格
            }
        ...
        new HtmlWebpackPlugin({
            template: './src/html/isomorphism-text.html'
            , filename: 'isomorphism-text.html'//可以使用hash命名
            , title: 'isomorphism-text'
            , inject: 'body'//脚本包含到body 也可以写到head里面
            , chunks: ['manifest', 'commons','isomorphism-text']//指定当前模板需要打入哪些js模块
            , minify: {//启用代码代码压缩
                removeComments: false,//移除注释
                collapseWhitespace: false//移除空格
            }
        ...
        new HtmlWebpackPlugin({
            template: './src/html/mutation.html'
            , filename: 'mutation.html'//可以使用hash命名
            , title: 'mutation'
            , inject: 'body'//脚本包含到body 也可以写到head里面
            , chunks: ['manifest', 'commons', 'mutation']//指定当前模板需要打入哪些js模块
            , minify: {//启用代码代码压缩
                removeComments: false,//移除注释
                collapseWhitespace: false//移除空格
            }
        ...
        new HtmlWebpackPlugin({
            template: './src/html/encrypt.html'
            , filename: 'encrypt.html'//可以使用hash命名
            , title: 'encrypt'
            , inject: 'body'//脚本包含到body 也可以写到head里面
            , chunks: ['manifest', 'commons', 'encrypt']//指定当前模板需要打入哪些js模块
            , minify: {//启用代码代码压缩
                removeComments: false,//移除注释
                collapseWhitespace: false//移除空格
            }
        ...
        new HtmlWebpackPlugin({
            template: './src/html/view-onscroll.html'
            , filename: 'view-onscroll.html'//可以使用hash命名
            , title: 'view-onscroll'
            , inject: 'body'//脚本包含到body 也可以写到head里面
            , chunks: ['manifest', 'commons','view-onscroll']//指定当前模板需要打入哪些js模块
            , minify: {//启用代码代码压缩
                removeComments: false,//移除注释
                collapseWhitespace: false//移除空格
            }
    ...
    devServer: {
        ...
        port: 8080, // 如果是小于1000的端口号，是需要sudo权限的，启用方式 sudo node server.js即可(可使用默认80端口)
        proxy: {
            '/content': {
                ...
                secure: false,
                onProxyReq: (proxyReq) => {
                    console.log('------------------------ onProxyReq: m.dianping.com ------------------------');
                    proxyReq.setHeader('Host', 'm.dianping.com');
                    proxyReq.setHeader('Origin', 'http://m.dianping.com');
                },
                onError(err, req, res) {
                    res.writeHead(500, {
